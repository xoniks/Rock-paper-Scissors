...
class HumanPlayer(Player):
    def move(self):
        move = input(
            "Enter a move ['rock', 'paper', 'scissors'] : "
        )  # makes user select a player and ensures the move is valid
...
class CyclePlayer(Player):
    def move(self):
        if self.memory:  # the memory has some store value
            ...
            )  # returns the next on in the cycle
        else:
            return choice(moves)
...
class Game:
    ...
    def play_round(self):
        ...
        return (
            1 if one_win else 2 if two_win else 0
        )  # returns the winners number and zero if tie
    ...
    def play_game(self):
        ...
        for round in range(3):
            ...
            print(
                f"Player{win_status} wins" if win_status != 0 else "Players draw"
            )  # displays the winning mesaage
        print(
            f"""
...
Players tie {game_stat.count(0)} time(s)
        ...
        if game_stat.count(1) > game_stat.count(2):
            print("PLAYER 1 WINS")
        elif game_stat.count(1) < game_stat.count(2):
            print("PLAYER 2 WINS")
        else:
            print("DRAW")
        print("Game over!")
